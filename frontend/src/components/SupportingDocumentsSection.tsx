...
export default function SupportingDocumentsSection({
  ...
  // Mandatory types — show row whether uploaded or not
  for (const dt of mandatoryDocTypes) {
    rows.push({
  ...
  // Extra attachments (beyond first per mandatory type)
  for (const att of attachments) {
    if (!shownInMandatoryRows.has(String(att.id))) {
      const dt = docTypes.find((d) => d.code === att.docType);
      rows.push({
        key: `extra-${att.id}`,
        docType: dt || { code: att.docType, name: att.docType, mandatoryFor: null },
        attachment: att,
        mandatory: false,
      });
    }
